...
public class addressBook { // Creates address book class
	private ArrayList<address> addressContent = new ArrayList<address>(); // Creates specific array list that will be used as master list for program.
	int currentSelection = -1; // Currect Selection set to defualt value.

	public addressBook() { // This address book function creates three stock contacts to initially populate the address book.
		addressContent.add(new address("99999987590", "John", "Smith", "900 Sylvan"));
		addressContent.add(new address("1082907812", "Donald", "Trump", " 1600 Pennsylvania"));
		addressContent.add(new address("111987252", "Some", "Guy", "101 Nowhere"));
	...
	public void search() { // Search function finds existing contact based on user input. If user input contacts first name, last name, OR phone number of contact, function will display that contact.
		...
		for (int i = 0; i < addressContent.size(); i++) {
			if (addressContent.get(i).phoneNumber.compareTo(userInput) == 0 // If statement compares each element (first name, last name, and phone number) of each item in array list to user input.
					|| addressContent.get(i).firstName.compareTo(userInput) == 0
					|| addressContent.get(i).firstName.compareTo(userInput) == 0
					|| addressContent.get(i).lastName.compareTo(userInput) == 0) {
				currentSelection = i; // Search sets currentSelection value to contact whose element matched the user input.
	...
	public void deleteCurrentSelection() { // Function deletes currentSelection contact from Array list. currentSelection contact is determined by using search function.
	...
	public void updateContact() { // updateContact function updates individual elements of already existing contacts.
		if (currentSelection == -1) {
			System.out.print("ERROR: Please use search (option 5) to find a contact to update.");
		} else {
			
			// Function goes through each individual element of contact. If element does not need to be changed, user hits ENTER. If element does need to be changed, the user instead enters the new value for stated element.
			addressContent.get(currentSelection).printAddress();
